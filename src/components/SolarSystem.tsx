...
export function SolarSystem({ onPlanetClick }: SolarSystemProps) {
  ...
  const renderLabel = (planet: PlanetData) => {
    if (language === 'en') {
      return <span className="text-[10px] md:text-xs font-medium whitespace-nowrap">{planet.name.en}</span>;
    }
    if (language === 'ur') {
      return <span className="font-urdu text-[10px] md:text-xs whitespace-nowrap">{planet.name.ur}</span>;
    }
    return (
  ...
  return (
    <div className="space-y-4">
      {/* Full-width rectangular orbit viewer */}
      <div
        ref={containerRef}
      ...
      >          {/* Sun */}
          <div className="absolute" style={{ width: 0, height: 0 }}>
            ...
            {/* Sun label */}
            <div
              className="absolute left-1/2 -translate-x-1/2 text-center whitespace-nowrap px-2 py-0.5 rounded-full"
              style={{ 
                top: 32, 
                color: '#ffcc00', 
                fontSize: '10px', 
          ...
          {/* Orbits and Planets */}
          {planets.map((planet) => {
            const startAngleRad = (planet.startAngle * Math.PI) / 180;
                        ...
                        {selectedId === planet.id && (
                          <div
                            ...
                            style={{
                              ...
                              boxShadow: '0 0 15px #60a5fa, 0 0 30px #3b82f680',
                              margin: '-6px',
                            }}
                          />
                        )}
